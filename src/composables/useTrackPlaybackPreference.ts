...
export function useTrackPlaybackPreference(options: {
  ...
  let currentContext: Context = { trackId: null, userId: null, key: '' }
  ...
  function currentScopePreference(): TrackPlaybackPreference | null {
    if (!currentContext.trackId || !currentContext.userId || !currentContext.key) return null
    return {
      track_id: currentContext.trackId,
      user_id: currentContext.userId,
      scope: options.scope(),
      gain_db: clampGainDb(gainDb.value),
      updated_at: null,
    }
  }

  async function flushSave(context: Context = currentContext): Promise<void> {
    if (!pendingSave || !context.trackId || !context.userId || !context.key) return

    pendingSave = false
    try {
      const saved = await trackApi.setPlaybackPreference(context.trackId, options.scope(), {
        gain_db: clampGainDb(gainDb.value),
      })
      ...
      const fallback = currentScopePreference()
  ...
  async function loadPreference(): Promise<void> {
    ...
    if (!enabled || trackId == null || userId == null) {
      currentContext = { trackId: null, userId: null, key: '' }
      isHydrating = true
      gainDb.value = 0
      isHydrating = false
      pendingSave = false
    ...
    currentContext = { trackId, userId, key }
