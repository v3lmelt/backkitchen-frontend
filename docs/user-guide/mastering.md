...
主催和母带师可在专辑设置中分别设置母带源音频和母带成品的 WAV、采样率与采样格式要求。母带工作台的「单曲音频规格」可按类别继承、自定义或关闭检查。

母带修订上传新源音频、上传母带、确认交付及终审批准时，若规格不符，按钮旁显示「音频规格不符」。点击后可查看实际与要求规格，选择「取消」或「仍然继续」。每次操作单独确认；双方终审也各自确认。请求修订与退回不触发规格确认。原有上传、确认交付和终审流程保持不变。

上传前在浏览器读取文件头；上传后以服务端探测为准。暂时无法核验时显示「规格暂未核验」，仍可继续操作。外部分轨链接不适用文件规格检查。

技术信息默认显示格式摘要，展开后可查看普通峰值、True Peak、Integrated LUFS、LRA、DC Offset、分声道结果及分析器版本。数据跟随正在查看的文件版本。响度与峰值不参与格式判断，也不会自动套用广播响度目标。静音、低于门限或过短的测量显示明确状态；少于 3 秒不提供 LRA，3–60 秒的 LRA 标注不建议用于短音频评价。
