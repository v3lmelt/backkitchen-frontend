...
点击“最终拒绝”后，曲目进入最终拒绝状态，不能通过普通重新投稿操作恢复。执行前应先下载需要保留的文件，不要依赖平台长期保留被拒绝曲目的音频。

只应在确定该曲目不再参与当前专辑时使用。执行前应确认曲师已经了解原因。

> 当前确认界面提示源音频文件将在 1 天后删除；具体保留期限以平台管理员说明为准。执行前请先下载需要保留的文件。
