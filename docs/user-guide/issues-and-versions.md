...
只有多人同行评审中的“仅内部”能够形成评审侧内部问题。单人评审、主催审批和终审只支持公开问题，不会显示“仅内部”；旧页面或其他客户端在这些阶段提交“仅内部”时，平台会拒绝请求且不会创建问题。即使在多人同行评审中，“仅内部”也主要保证问题对投稿曲师隐藏，不应理解为严格限制为指定评审人可见。
...
“仅内部”主要表示对投稿曲师隐藏，不是严格限制为指定评审人可见。单人评审、主催审批和终审只支持公开问题；平台会拒绝在这些阶段提交“仅内部”的请求。

## 查看源文件版本

“制作周期”用于区分一次完整制作与重新进入流程后的新一轮制作。初始投稿和拒绝后的重新提交会建立新的周期；重新开启时是否增加周期取决于选择返回的阶段。

每次初始投稿或上传修订版时，平台都会建立新的源文件版本记录。版本记录包含：
...
在曲目页面选择历史源版本后，可以在文件仍处于保留期内时播放、下载或与当前版本比较。历史版本记录不会因上传新版本而被覆盖。
